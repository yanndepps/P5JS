...
class SimpleLines extends Layer {
	constructor() {
		super()
		this.numSteps    = randomSelectTwo() ? this.stepOut : int(this.stepOut * 1.25)
...
class CenterShape extends Layer {
	constructor() {
		...
		this.shapeSize   = floor(random(this.stepOut / 2, this.stepOut -2)) * this.singleStep
	}
...
class RingOfShapes extends Layer {
	constructor() {
		...
		if (this.steps < this.stepOut / 2) {
			this.radius = floor(random(1, this.steps)) * this.singleStep
		} else if (this.steps > this.stepOut / 2) {
			this.radius = floor(random(1, this.stepOut - this.steps)) * this.singleStep
		} else {
			this.radius = floor(random(1, (this.stepOut / 2) + 1)) * this.singleStep
		}
	}
...
class SteppedHexagons extends Layer {
	constructor() {
		super()
		this.numSteps = randomSelectTwo() ? this.stepOut : this.stepOut * 1.25
