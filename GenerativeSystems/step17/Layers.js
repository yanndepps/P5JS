...
const setState = (state) => {
	...
	state.singleStep = (CRYSTAL_SIZE / 2) / state.stepOut,
...
const simpleLines = (state) => {
  state.numSteps = randomSelectTwo() ? state.stepOut : int(state.stepOut * 1.25)
...
const centerShape = (state) => {
  state.randomShape = random(1)
  state.shapeSize = floor(random(state.stepOut / 2, state.stepOut - 2)) * state.singleStep
...
const ringOfShapes = (state) => {
  state.steps = floor(random(1, state.stepOut))
  ...
  if (state.steps < state.stepOut / 2) {
    state.radius = floor(random(1, state.steps)) * state.singleStep
  } else if (state.steps > state.stepOut / 2) {
    state.radius = floor(random(1, state.stepOut - state.steps)) * state.singleStep
  } else {
    state.radius = floor(random(1, (state.stepOut / 2) + 1)) * state.singleStep
  }
...
const steppedHexagons = (state) => {
  state.numSteps = randomSelectTwo() ? state.stepOut : state.stepOut * 1.25
